...
const Calendario = () => {
  ...
  const pendingCount = obligations.filter((o) => o.status === "pendente").length;
  const completedCount = obligations.filter((o) => o.status === "concluido").length;
  const overdueCount = obligations.filter((o) => o.status === "atrasado").length;
    ...
    <div className="space-y-6">
      <FadeIn>
        <PageHeader
          ...
          description="Visualize e gerencie prazos de obrigações contratuais"
        />
      </FadeIn>
              ...
              <AnimatedCardContent>
                ...
                <div className="grid grid-cols-7 gap-1">
                  {paddingDays.map((_, index) => (
                    <div key={`padding-${index}`} className="min-h-[72px]" />
                  ))}
                  {daysInMonth.map((day, index) => {
                    ...
                    const hasPending = dayObligations.some((o) => o.status === "pendente");
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        className={cn(
                          "p-2 text-center rounded-lg transition-all relative min-h-[72px] flex flex-col items-center border border-transparent",
