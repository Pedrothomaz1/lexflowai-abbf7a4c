...
Deno.test("contratos: admin of Org B sees no Org A contracts", async () => {
...
Deno.test("contratos: insert with foreign organization_id is blocked", async () => {
  const seed = await bootstrap();
...
Deno.test("audit_logs: non-admin cannot read", async () => {
  await bootstrap();
...
Deno.test("fornecedores: cross-org select returns zero", async () => {
  const seed = await bootstrap();
  const b = await signInAs("adminB");
...
Deno.test("contratos update with insufficient role yields no row (silent RLS)", async () => {
  const seed = await bootstrap();
  const analista = await signInAs("analistaA");
