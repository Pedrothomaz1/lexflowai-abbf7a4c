...
Deno.test("storage: admin of Org A can read own org file", async () => {
...
Deno.test("storage: admin of Org B cannot read Org A file (cross-org)", async () => {
  const seed = await bootstrap();
...
Deno.test("storage: admin of Org B cannot list Org A folder", async () => {
  const seed = await bootstrap();
  const b = await signInAs("adminB");
...
Deno.test("storage: cross-org upload is rejected", async () => {
  const seed = await bootstrap();
  const a = await signInAs("adminA");
...
Deno.test("storage: anonymous cannot read avatars/* (legacy public path closed)", async () => {
