...
serve(async (req) => {
  ...
  try {
    ...
    const { data: contrato } = await supabase.from('contratos').select('id, organization_id, created_by').eq('id', contratoId).maybeSingle();
    ...
    console.error('ia-resumo-executivo error', e);
