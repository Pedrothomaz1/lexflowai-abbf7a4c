...
BEGIN
  ...
  IF c.titulo IS NULL OR length(trim(c.titulo)) = 0 THEN v_faltantes := array_append(v_faltantes, 'titulo'); END IF;
  IF c.tipo IS NULL THEN v_faltantes := array_append(v_faltantes, 'tipo'); END IF;
  IF c.fornecedor_id IS NULL THEN v_faltantes := array_append(v_faltantes, 'fornecedor_id'); END IF;
  IF c.data_inicio IS NULL THEN v_faltantes := array_append(v_faltantes, 'data_inicio'); END IF;
  IF c.data_fim IS NULL THEN v_faltantes := array_append(v_faltantes, 'data_fim'); END IF;
  IF c.valor_total IS NULL THEN v_faltantes := array_append(v_faltantes, 'valor_total'); END IF;
  IF c.departamento_responsavel IS NULL OR length(trim(c.departamento_responsavel)) = 0 THEN
    v_faltantes := array_append(v_faltantes, 'departamento_responsavel');
  END IF;
  IF c.centro_custo IS NULL OR length(trim(c.centro_custo)) = 0 THEN
    v_faltantes := array_append(v_faltantes, 'centro_custo');
  END IF;
  IF c.renovacao_automatica = true AND (c.dias_aviso_nao_renovacao IS NULL OR c.dias_aviso_nao_renovacao <= 0) THEN
    v_faltantes := array_append(v_faltantes, 'dias_aviso_nao_renovacao');
  END IF;
  ...
  IF NOT v_tem_original THEN
    v_faltantes := array_append(v_faltantes, 'anexo_original');
  END IF;
...
BEGIN
  ...
  IF c.nivel_risco IN ('alto','critico') THEN
    IF c.due_diligence_status IS DISTINCT FROM 'concluida' THEN
      v_faltantes := array_append(v_faltantes, 'due_diligence_concluida');
    END IF;
    IF c.sanction_check_status IS DISTINCT FROM 'aprovado' THEN
      v_faltantes := array_append(v_faltantes, 'sanction_check_aprovado');
    END IF;
  END IF;
