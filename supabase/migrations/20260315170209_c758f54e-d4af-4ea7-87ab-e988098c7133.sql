-- Recreate fornecedores_safe with SECURITY INVOKER so RLS is enforced
DROP VIEW IF EXISTS public.fornecedores_safe;
CREATE VIEW public.fornecedores_safe
WITH (security_invoker = true)
AS
SELECT 
  id, nome, cnpj, cidade, estado, email, telefone, is_active,
...
-- Recreate profiles_safe with SECURITY INVOKER so RLS is enforced
